...
PcAuxData$
    methods(
        ...
        setStatus = function(step = "start")
        {
            ...
            stCall <- sum(lapply(call, function(x) is.null(x)))
        ...
        castData = function()
        {
            ...
            tempLength <- ncol(data)
            for(i in 1 : seq_len(tempLength)) {
                data[ , i] <<-
                    switch(typeVec[i],
                           drop       =              data[ , i],
                           id         =              data[ , i],
        ...
        transformMiData = function()
        {
            ...
            if(compFormat == "long") {
                ...
                tempLength <- nrow(data)
                .id  <- rep(c(1 : seq_len(tempLength)), nImps       )
                miDatasets <<-
        ...
        computeInteract = function()
        {
            ...
            intVars <<- as.character(lapply(varCombs, paste0, collapse = "."))
            ...
            levVec   <-  lapply(interact, countLevels)
            ...
            if(intMeth > 1)  
                tempLength <- ncol(interact)
                for(v in 1 : seq_len(tempLength))
                    interact[ , v] <<-
                        .lm.fit(y = interact[ , v],
